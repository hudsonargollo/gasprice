...
exports.HuiduProtocolService = void 0;
const logger_1 = require("../utils/logger");
class HuiduProtocolService {
    constructor() {
        // Protocol constants
        this.STX = 0x02; // Start of text
        this.ETX = 0x03; // End of text
        this.CMD_PRICE_UPDATE = 0x31; // Command for price update
        // CRC16-CCITT polynomial
        this.CRC16_POLY = 0x1021;
    }
    /**
     * Create a Huidu protocol frame for price updates
     */
    createPriceUpdateFrame(prices, panelId) {
        try {
            ...
            logger_1.logger.debug(`Created Huidu frame: ${frame.toString('hex')}`);
            return frame;
        }
        catch (error) {
            logger_1.logger.error('Error creating Huidu price update frame:', error);
            throw error;
        }
    }
    /**
     ...
     */
    createPriceUpdateCommand(prices, panelId) {
        try {
            const dataPayload = this.createPriceDataPayload(prices, panelId);
            ...
            logger_1.logger.debug(`Created Huidu command:`, {
            ...
            logger_1.logger.error('Error creating Huidu command:', error);
            throw error;
        }
    }
    /**
     ...
     */
    createPriceDataPayload(prices, panelId) {
    ...
    parseFrame(frame) {
        ...
        if (!validation.isValid) {
            logger_1.logger.error('Frame validation failed:', validation.error);
            return null;
        }
        try {
            ...
            logger_1.logger.error('Error parsing frame:', error);
            return null;
        }
    }
    /**
     ...
     */
    createCustomFrame(command, data) {
...
exports.HuiduProtocolService = HuiduProtocolService;
